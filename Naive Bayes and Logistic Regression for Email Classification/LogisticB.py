...
#ham  = func(path = "./Dataset2/enron1/train/ham", cl = 1)
#spam = func(path = "./Dataset2/enron1/train/spam",cl = 0)
#
#train_data = ham.append(spam).fillna(0)
#train_data.index = range(len(train_data.index))
#
#train_data.to_csv(r'F:\UTD\Machine Learning\HW2\Dataset2_train.csv', index=False)
#train_data = pd.read_csv("Dataset3_trainB.csv")
#
#
#
#t_ham = func(path = "./Dataset2/enron1/test/ham", cl = 1)
#t_spam = func(path = "./Dataset2/enron1/test/spam",cl = 0)
#
#test = t_ham.append(t_spam).fillna(0)
#test.index = range(len(test.index))
#
#test.to_csv(r'F:\UTD\Machine Learning\HW2\Dataset2_test.csv', index=False)
#test = pd.read_csv("Dataset3_testB.csv")
...
#    J_1 = 1/m*(X.transpose() @ (pred - y))[1:] - alpha*(Lambda/m)*theta[1:]
    grad = np.dot(X.transpose(), (y-pred))
...
#Lambda = 3
#    
#theta , J_history = gradientAscent(X_train,y_train,theta,0.01, 1500, Lambda)
...
#test_ham = func_message(path = "./Dataset3/enron4/test/ham", classification = 1)
#
#test_spam = DataFrame({'Message': [], 'class': []})
#test_spam = func_message(path = "./Dataset3/enron4/test/spam", classification = 0)
#
#test_data = test_ham.append(test_spam)
#test_data.index = range(len(test_data.index))
###
##
#test_data.to_csv(r'F:\UTD\Machine Learning\HW2\Dataset3_test_m.csv', index=False)
#test_data = pd.read_csv("Dataset3_test_m.csv")
...
#    theta , J_history = gradientAscent(X_train,y_train,theta,0.2, 1000, Lambda)
    ...
    print("Train Accuracy:", (sum(p==y_train)/len(y_train) *100)[0],"%")
    p=classifierPredict(theta,X_test)
    print("Validation Accuracy:", (sum(p==y_test)/len(y_test) *100)[0],"%")
